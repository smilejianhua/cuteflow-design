...
/**
 * 工作项对象。 对该对象的状态字段作如下规定：小于5的状态为“活动”状态，大于等于5的状态为“非活动”状态。
 */
public interface Task extends InstanceState {
	 ...
	 * @return
	 */
	public ActivityInstance getTask();
	 ...
	 */
	public void sign() throws EngineException;
	 ...
	 * 
	 * @return 如果签收成功，则返回一个新的WorkItem对象；否则返回null
	 */
	public Task claim() throws EngineException;

	/**
	 * 对已经结束的工作项执行取回操作<br/>
	 * 只有满足如下约束才能正确执行取回操作：<br/>
	 * 1) 下一个Activity只有Form类型的Task,没有Tool类型和Subflow类型的Task</br> 2)
	 * 下一个环节的所有WorkItem还没有被签收，都处于Initialized状态，<br/>
	 * 如果在本WorkItem成功执行了jumpTo操作或者loopTo操作，只要满足上述条件，也可以 成功执行withdraw。<br/>
	 * 该方法和IWorkflowSession.withdrawWorkItem(String workItemId)等价。
	 * 
	 * @return 如果取回成功，则创建一个新的WorkItem 并返回该WorkItem
	 */
	public Task withdraw() throws EngineException;
	 ...
	 */
	public void reject(String comments) throws EngineException;
	 ...
	 * 2、判断TaskInstance对应的ActivityInstance是否可以结束。如果ActivityInstance的complete
	 * strategy 为ANY，或者，complete
	 * strategy为ALL且他的所有的TaskInstance都已经结束，则结束当前ActivityInstance<br/>
	...
	/**
	 * 结束当前WorkItem并由工作流引擎根据流程定义决定下一步操作。引擎的执行规则如下<br/>
	 * 1、工作流引擎首先判断该WorkItem对应的TaskInstance是否可以结束。
	 * 如果TaskInstance的assignment策略为ANY，或者，assignment策略为ALL且它所有的WorkItem都已经完成
	 * 则结束当前TaskInstance<br/>
	 * 2、判断TaskInstance对应的ActivityInstance是否可以结束。如果ActivityInstance的complete
	 ...
	 * @throws KernelException
	 */
//	public void complete(AssignmentService assignmentService, String comments)
//			throws EngineException;
	 ...
	 * 2)当前Task的assignment为Task.ANY。或者当前Task的assignment为Task.ALL(汇签)，
	 * 且本WorkItem结束后可以使得TaskInstance结束；与之相反的情况是，
	 * 尚有其他参与汇签的操作者没有完成其工作项，这时engine拒绝跳转操作<br/>
	 * 3)当前TaskInstance结束后,可以使得当前的ActivityInstance结束。与之相反的情况是，
	 * 当前Activity包含了多个Task，且Activity的Complete Strategy是ALL，
	 ...
	 *            下一个环节的id
	 * @param dynamicAssignmentHandler
	 *            可以通过该参数指定下一个环节的Actor，如果这个参数不为空，则引擎忽略下一个环节的Task定义中的AssignmentHandler
	 * @param comments
	 *            备注信息
	 * @throws org.fireflow.engine.EngineException
	 * @throws org.fireflow.kenel.KenelException
	 */
//	public void jumpTo(String targetActivityId,
//			AssignmentService assignmentService, String comments)
//			throws EngineException;
	 ...
	 * @return 新创建的工作项
	 */
	public Task reasignTo(String actorId) throws EngineException;
	 ...
	 * @return 新创建的工作项
	 */
	public Task reasignTo(String actorId, String comments)
			throws EngineException;
}
